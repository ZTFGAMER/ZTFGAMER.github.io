...
import { tierStarLevelIndex } from './SynthesisLogic'
...
export function refreshPlayerStatusUI(
  ...
  if (ctx.playerStatusDailySkillStar) {
    ctx.playerStatusDailySkillStar.visible = deps.shouldShowHeroDailySkillReadyStar()
  }
}
...
export function layoutPlayerStatusPanel(ctx: ShopSceneCtx): void {
  ...
  ctx.playerStatusCon.y = PvpContext.isActive() ? getDebugCfg('battleZoneY') - 200 : getDebugCfg('shopPlayerStatusY')
