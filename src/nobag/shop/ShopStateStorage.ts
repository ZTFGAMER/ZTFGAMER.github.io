...
import { SHOP_STATE_STORAGE_KEY } from '@/nobag/core/NobagRunState'
import { getAllItems, getRunClassItemPoolIds, setRunClassItemPoolIds } from '@/nobag/core/NobagDataLoader'
import type { ShopSlot } from '@/nobag/shop/ShopManager'
import type { GridSystem } from '@/nobag/common/grid/GridSystem'
import type { GridZone } from '@/nobag/common/grid/GridZone'
...
export type ApplySavedShopStateCallbacks = {
  toVisualTier: (tier?: import('@/nobag/shop/ShopManager').TierKey, star?: 1 | 2) => string | undefined
  ...
  ctx.pendingHeroPeriodicRewards = Array.isArray(state.pendingHeroPeriodicRewards)
    ? state.pendingHeroPeriodicRewards
      .map((row) => {
        ...
        const tier = (String(row?.tier ?? 'Bronze') as import('@/nobag/shop/ShopManager').TierKey)
        ...
        const picks = Array.isArray(entry?.picks)
          ? entry.picks
            .map((pick) => {
              ...
              const tier = (String(pick?.tier ?? 'Bronze') as import('@/nobag/shop/ShopManager').TierKey)
