...
import type { TierKey } from '@/nobag/shop/ShopManager'
...
import { isSkillItemDefId } from '@/nobag/common/skills/SkillItemDefs'
...
import { planUnifiedSqueeze } from '@/nobag/common/grid/SqueezeLogic'
import { planAutoPack, type PackItem, type PackPlacement } from '@/nobag/common/grid/AutoPack'
...
export function applySellButtonState(ctx: ShopSceneCtx): void {
  ...
  if (ctx.sellBtnHandle?.container) ctx.sellBtnHandle.container.visible = false
  if (ctx.refreshBtnHandle?.container) ctx.refreshBtnHandle.container.visible = false
...
export function getGridDragDiscardAreaLocalRect(): { x: number; y: number; w: number; h: number } {
  const x = 20
  const y = Math.round(CANVAS_H * 0.11)
  const w = CANVAS_W - x * 2
  const baseH = Math.round(CANVAS_H * 0.29)
  const h = Math.min(CANVAS_H - y, baseH + 200)
  return { x, y, w, h }
}

export function isOverGridDragSellArea(gx: number, gy: number): boolean {
  const stage = getApp().stage
  const rect = getGridDragDiscardAreaLocalRect()
  const lt = stage.toGlobal({ x: rect.x, y: rect.y })
  const rb = stage.toGlobal({ x: rect.x + rect.w, y: rect.y + rect.h })
  const minX = Math.min(lt.x, rb.x)
  const maxX = Math.max(lt.x, rb.x)
  const minY = Math.min(lt.y, rb.y)
  const maxY = Math.max(lt.y, rb.y)
...
function makeGridDragInternalDeps(ctx: ShopSceneCtx) {
  return {
    ...
    getGridDragDiscardAreaLocalRect: () => getGridDragDiscardAreaLocalRect(),
  }
}
