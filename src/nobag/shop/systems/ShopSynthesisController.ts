...
import type { ItemSizeNorm, PlacedItem } from '@/nobag/common/grid/GridSystem'
...
import { getRunClassItemPoolIds } from '@/nobag/core/NobagDataLoader'
import {
  TIER_ORDER,
  nextTierLevel,
  tierStarLevelIndex,
...
import { shouldTriggerSkill48ExtraUpgrade } from '@/nobag/common/skills/GoldSkillRules'
import { toVisualTier, getSizeCellDim } from '../ShopMathHelpers'
import { getConfig as getDebugCfg } from '@/nobag/config/debugConfig'
import { getShopUiColor } from '@/nobag/config/colorPalette'
...
export function clearBackpackSynthesisGuideArrows(ctx: ShopSceneCtx): void {
  ctx.backpackView?.setDragGuideArrows([])
  ctx.battleView?.setDragGuideArrows([])
  ctx.levelQuickRewardView?.setDragGuideArrows([])
}
...
export function synthesizeTarget(
  ...
  const filterCrossSynthesisPool = (list: ItemDef[]): ItemDef[] => {
    ...
    const pickFromPair = getDebugCfg('gameplaySameArchetypeCrossSynthesisPickFromPair') >= 0.5
    if (pickFromPair && shouldExcludeSameArch) {
      const pairCandidates: ItemDef[] = []
      pairCandidates.push(sourceDef)
      if (targetDef.id !== sourceDef.id) pairCandidates.push(targetDef)
