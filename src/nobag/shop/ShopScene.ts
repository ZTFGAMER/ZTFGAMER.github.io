...
import { SceneManager, type Scene } from '@/nobag/core/NobagSceneManager'
import { getApp } from '@/nobag/core/NobagAppContext'
import { getConfig, getAllItems, setRunClassItemPoolIds } from '@/nobag/core/NobagDataLoader'
import {
...
import type { ItemSizeNorm } from '@/nobag/common/grid/GridSystem'
import { CELL_HEIGHT, CELL_SIZE } from '@/nobag/common/grid/GridZone'
import type { ItemDef } from '@/nobag/common/items/ItemDef'
import { parseTierName } from '@/nobag/common/items/ItemTierStats'
import { ShopManager, getDailyGoldForDay, type TierKey } from '@/nobag/shop/ShopManager'
import { getConfig as getDebugCfg } from '@/nobag/config/debugConfig'
import { PhaseManager } from '@/nobag/core/NobagPhaseManager'
import { clearBattleSnapshot, getBattleSnapshot, setBattleSnapshot, type BattleSnapshotBundle } from '@/nobag/battle/BattleSnapshotStore'
import { PvpContext } from '@/nobag/pvp/NobagPvpContext'
// getOpponentFromAlive moved to PvpPanel.ts
import { clearBattleOutcome, consumeBattleOutcome } from '@/nobag/battle/BattleOutcomeStore'
...
import { createShopSceneCtx, type ShopSceneCtx, type StarterTutorialStep } from './ShopSceneContext'
import {
  nextId,
  instanceToDefId,
...
import * as HeroSystem from './systems/ShopHeroSystem'
import {
  resetEventSelectionCounters,
  resetDayEventState,
  resetFutureEventState,
} from './systems/ShopEventSystem'
import * as EventSystem from './systems/ShopEventSystem'
import {
...
import * as QuickBuySystem from './systems/QuickBuySystem'
import {
} from './ui/PlayerStatusUI'
import * as PlayerStatusUI from './ui/PlayerStatusUI'
import {
  hasPickedSkill,
...
import { getSkillItemDefById, getSkillPickById } from '@/nobag/common/skills/SkillItemDefs'
import {
  stopFlashEffect,
...
import { CANVAS_W } from '@/nobag/config/layoutConstants'
import { getShopUiColor, getClassColor } from '@/nobag/config/colorPalette'
import { getItemIconUrl } from '@/nobag/core/NobagAssetPath'
import {
...
import * as GridInventory from './systems/ShopGridInventory'
import type { GridInventoryCallbacks, OwnedPlacedItem } from './systems/ShopGridInventory'
import * as PostBattle from './systems/ShopPostBattle'
import type { PostBattleCallbacks } from './systems/ShopPostBattle'
import * as SynthesisCtrl from './systems/ShopSynthesisController'
import type { SynthesisCallbacks, SynthesizeResult, SynthesisTarget } from './systems/ShopSynthesisController'
import * as RewardSystem from './systems/ShopRewardSystem'
import type { RewardSystemCallbacks } from './systems/ShopRewardSystem'
import * as DebugLayout from './ui/ShopDebugLayout'
import type { DebugLayoutCallbacks } from './ui/ShopDebugLayout'
import * as InstanceMgr from './systems/ShopInstanceManager'
import * as PurchaseLogic from './systems/ShopPurchaseLogic'
import type { PurchaseCallbacks } from './systems/ShopPurchaseLogic'
import * as ItemGrant from './systems/ShopItemGrant'
import type { ItemGrantCallbacks } from './systems/ShopItemGrant'
import * as DragSystem from './systems/ShopDragSystem'
import type { ShopDragDeps } from './systems/ShopDragSystem'
import { refreshUpgradeHints } from './ui/ShopUpgradeHints'
import * as UIBuilders from './ui/ShopUIBuilders'
import type { TopAreaUICallbacks, ButtonRowUICallbacks } from './ui/ShopUIBuilders'
import * as EventBusSetup from './ShopEventBusSetup'
import type { EventBusSetupCallbacks } from './ShopEventBusSetup'
import * as PanelInit from './ShopPanelInitializer'
...
function applyPhaseUiVisibility(ctx: ShopSceneCtx = _ctx): void {
  ...
  if (ctx.specialShopBackpackViewActive) {
    ...
    if (ctx.refreshBtnHandle) ctx.refreshBtnHandle.container.visible = false
    if (ctx.sellBtnHandle) ctx.sellBtnHandle.container.visible = false
    if (ctx.phaseBtnHandle) ctx.phaseBtnHandle.container.visible = !hasBlockingOverlay
  }
...
function setBaseShopPrimaryButtonsVisible(visible: boolean, ctx: ShopSceneCtx = _ctx): void {
  const showPhaseBtn = visible
    && !ctx.specialShopBackpackViewActive
    && !ctx.skillDraftOverlay
    && !ctx.eventDraftOverlay
    && !ctx.specialShopOverlay
    && !ctx.levelQuickRewardOverlay
  if (ctx.bpBtnHandle) ctx.bpBtnHandle.container.visible = visible
  if (ctx.refreshBtnHandle) ctx.refreshBtnHandle.container.visible = false
  if (ctx.sellBtnHandle) ctx.sellBtnHandle.container.visible = false
  if (ctx.phaseBtnHandle) ctx.phaseBtnHandle.container.visible = showPhaseBtn
}
...
function refreshShopUI(ctx: ShopSceneCtx = _ctx): void {
  ...
  const hasBlockingOverlay = !!(ctx.skillDraftOverlay || ctx.eventDraftOverlay || ctx.specialShopOverlay || ctx.levelQuickRewardOverlay)
  if (ctx.specialShopBackpackViewActive || hasBlockingOverlay) {
    setBaseShopPrimaryButtonsVisible(false)
  } else {
    setBaseShopPrimaryButtonsVisible(inShop)
  }
  if (!PvpContext.isActive()) saveShopStateToStorage(captureShopState(ctx))
}
type PoolCandidate = {
...
function setDay(day: number, ctx: ShopSceneCtx = _ctx): void {
  ...
  if (ctx.shopManager) {
    ...
    if (ctx.currentDay !== prevDay) {
      ...
      RewardSystem.enqueueRoundAutoQuickDrafts(ctx.currentDay, 3, ctx, makeRewardSystemCallbacks())
    }
  }
...
export const ShopScene: Scene = {
  ...
  onEnter() {
    ...
    if (restoredState) {
      ...
      grantSkill20DailyBronzeItemIfNeeded()
      RewardSystem.enqueueRoundAutoQuickDrafts(_ctx.currentDay, 3, _ctx, makeRewardSystemCallbacks())
    }
