...
import { getConfig, getAllItems, getAllItemsRaw } from '@/nobag/core/NobagDataLoader'
import { getWinTrophyState, getPlayerProgressState } from '@/nobag/core/NobagRunState'
import { resolveItemTierBaseStats } from '@/nobag/common/items/ItemTierStats'
...
import { clampPlayerLevel } from './ui/PlayerStatusUI'
import { levelToTierStar } from './systems/QuickBuySystem'
import type { ShopSceneCtx } from './ShopSceneContext'
import type { ItemEnchantmentKey } from '@/nobag/common/items/ItemEnchantment'
import { resolveItemEnchantmentEffectCn } from '@/nobag/common/items/ItemEnchantment'
...
export function buildBattleSnapshot(ctx: ShopSceneCtx, skillBarMoveStartAtMs?: number): BattleSnapshotBundle | null {
  ...
  const hpTable = getConfig().runRules?.playerBattleHpByDay
  const hpLast = Array.isArray(hpTable) && hpTable.length > 0 ? hpTable[hpTable.length - 1] : 500
  const hpIdx = Math.max(0, Math.min((hpTable?.length ?? 1) - 1, ctx.currentDay - 1))
  const playerBattleHp = Math.max(1, Math.round(Number(hpTable?.[hpIdx] ?? hpLast ?? 500)))
  return {
    day: ctx.currentDay,
...
function applyEnchantmentPreview(
  item: import('@/nobag/common/items/ItemDef').ItemDef,
