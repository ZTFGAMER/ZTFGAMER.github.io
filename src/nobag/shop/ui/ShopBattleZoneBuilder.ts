...
import { DragController } from '@/nobag/common/grid/DragController'
...
import { ShopPanelView } from '@/nobag/shop/ui/ShopPanelView'
import { getConfig as getDebugCfg } from '@/nobag/config/debugConfig'
import { getConfig as getGameCfg } from '@/nobag/core/NobagDataLoader'
import { getApp } from '@/nobag/core/NobagAppContext'
import { normalizeSize, type ItemDef } from '@/nobag/common/items/ItemDef'
import type { TierKey } from '@/nobag/shop/ShopManager'
import { Container, Text, type FederatedPointerEvent } from 'pixi.js'
import type { ItemInfoCustomDisplay } from '@/nobag/common/ui/SellPopup'
import {
  instanceToDefId,
...
export function buildBattleZoneUI(
  ...
  ctx.drag.onSpecialDrop = ({ instanceId, anchorGx, anchorGy, size, originCol, originRow, homeSystem, homeView, defId }) => {
    ...
    // 1) 拖到下方出售区域：直接出售
    // 无背包模式：命中出售区时优先出售，避免与战斗区底边格子判定重叠。
    // 变化石/转职石：命中出售区时优先允许出售，避免"无目标时无法出售"。
    if (overSellArea || forceDiscardForNeutralStone) {
      const ownedPlacedCount = (ctx.battleSystem?.getAllItems().length ?? 0) + (ctx.backpackSystem?.getAllItems().length ?? 0)
      if (homeView === ctx.levelQuickRewardView && ownedPlacedCount <= 0) {
        showHintToast('backpack_full_buy', '当前没有已持有物品，无法跳过或丢弃奖励', 0xffd48f, ctx)
        return false
      }
  ...
  ctx.drag.onDragMove = ({ instanceId, anchorGx, anchorGy, size }) => {
    ...
    const overSell = ctx.gridDragCanSell && ctx.gridDragSellHot
      ...
      const enchantDisplay = enchantment
        ...
        : undefined
      ctx.sellPopup.show(item, 0, 'none', toVisualTier(tier, star), undefined, 'detailed', undefined, customDisplay, enchantDisplay)
    ...
    if (!defId || !tier || (!nextTierLevel(tier, star) && !canLv7Morph)) {
      ...
      if (item && ctx.sellPopup) {
        ...
        const enchantDisplay = enchantment
          ...
          : undefined
        ctx.sellPopup.show(item, 0, 'none', toVisualTier(tier, star), undefined, getDefaultItemInfoMode(), undefined, undefined, enchantDisplay)
      }
      return
    }
    ...
    if (item && isNeutralTargetStone(item)) {
      ...
      if (target) {
        ...
        if (ctx.sellPopup) {
          ...
          ctx.sellPopup.show(item, 0, 'none', toVisualTier(tier, star), undefined, 'detailed', undefined, customDisplay)
        }
      }
      return
    }
