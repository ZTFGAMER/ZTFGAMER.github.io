...
import { BTN_RADIUS } from '@/nobag/config/layoutConstants'
...
export type StartGridDragButtonFlashDeps = {
  ...
  getGridDragDiscardAreaLocalRect: () => { x: number; y: number; w: number; h: number }
}
...
export function startGridDragButtonFlash(
  ...
  ctx.gridDragFlashTick = () => {
    ...
    if (ctx.gridDragSellZoneBg && ctx.gridDragSellZoneText) {
      const rect = deps.getGridDragDiscardAreaLocalRect()
      const hot = ctx.gridDragSellHot
      ctx.gridDragSellZoneBg.clear()
      ctx.gridDragSellZoneBg.roundRect(rect.x, rect.y, rect.w, rect.h, 16)
      ...
      ctx.gridDragSellZoneText.text = hot && ctx.gridDragNeutralDiscardHint
        ? '丢弃无法获得金币'
        : '拖动到此处丢弃'
      ctx.gridDragSellZoneText.x = rect.x + rect.w / 2
      ctx.gridDragSellZoneText.y = rect.y + rect.h / 2
    }
  }
...
export function stopGridDragButtonFlash(ctx: ShopSceneCtx, deps: StartGridDragButtonFlashDeps): void {
  ...
  if (ctx.phaseBtnHandle) ctx.phaseBtnHandle.container.visible = inShop && !hasBlockingOverlay
