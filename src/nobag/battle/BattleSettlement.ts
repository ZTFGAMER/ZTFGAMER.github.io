...
export class BattleSettlement {
  ...
  private layoutSettlementButtons(): void {
    ...
    if (this.settlementReplayBtn) {
      this.settlementReplayBtn.y = 44
      this.settlementReplayBtn.x = 100
    }
    if (this.settlementStatsBtn) {
      this.settlementStatsBtn.y = 44
  ...
  resolve(day: number, engine: CombatEngine, opts?: { applyRunState?: boolean }): void {
    ...
    const lifeResult = (applyRunState && !PvpContext.isActive() && winner === 'enemy')
    ...
    const shouldAddTrophy = applyRunState && !PvpContext.isActive() && (winner === 'player' || winner === 'draw')
    const trophyAfter = shouldAddTrophy ? addWinTrophy(trophyTarget) : trophyBefore
    if (applyRunState && !PvpContext.isActive()) {
    ...
    this.settlementGameOver = applyRunState && !PvpContext.isActive() && winner === 'enemy' && after.current <= 0 && !pveLastStandTriggered
    this.settlementFinalVictory = applyRunState && !PvpContext.isActive() && winner === 'player' && trophyAfter.wins >= trophyAfter.target
