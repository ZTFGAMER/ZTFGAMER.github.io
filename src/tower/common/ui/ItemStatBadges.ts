...
function parseArchetypes(item: ItemDef): ArchetypeKey[] {
  ...
  const hasWeapon = tags.some((t) => t === 'weapon' || t === '武器' || t === '战士' || t === '剑士' || t === '弓手' || t === '刺客' || t === '忍者' || t === '冰法师' || t === '中立' || t === 'neutral')
  if (!hasWeapon) return []

  const set = new Set<ArchetypeKey>()
  for (const tag of tags) {
    if (tag === 'warrior' || tag === '战士' || tag === '剑士') set.add('warrior')
    if (tag === 'archer' || tag === '弓手') set.add('archer')
    if (tag === 'assassin' || tag === '刺客' || tag === '忍者') set.add('assassin')
    if (tag === 'mage' || tag === '冰法师') set.add('mage')
