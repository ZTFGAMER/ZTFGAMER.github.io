...
import type { SkillArchetype } from '@/tower/common/items/ItemDef'
import type {
...
export class SkillTriggerSystem {
  ...
  minReducedCdMsFor(_item: CombatItemRunner): number {
    const cfgMin = Number(getConfig().towerDefenseRules?.reducedCooldownMinMs)
    const baseMin = Math.max(MIN_REDUCED_CD_MS, Number.isFinite(cfgMin) ? cfgMin : 500)
    return baseMin
  }
  ...
  applyBattleStartPassiveGrowths(): void {
    for (const owner of this.state.items) {
      ...
      const globalIntervalReduceLine = lines.find((s) => /所有物品间隔缩短/.test(s))
      if (globalIntervalReduceLine) {
        ...
        const pctBase = pctSeries
          ? Math.max(0, pickTierSeriesValue(pctSeries, tIdx)) / 100
          : Math.max(0, tierValueFromLine(globalIntervalReduceLine, tIdx)) / 100
        if (pctBase > 0) {
          for (const ally of this.state.items) {
            if (ally.side !== owner.side) continue
            if (ally.baseStats.cooldownMs <= 0) continue
            const allyDef = findItemDef(ally.defId)
            const isArcher = itemArchetype(allyDef) === '弓手'
            const pct = Math.min(0.95, pctBase * (isArcher ? 2 : 1))
            if (pct <= 0) continue
            ally.baseStats.cooldownMs = Math.max(
              this.minReducedCdMsFor(ally),
              Math.round(ally.baseStats.cooldownMs * (1 - pct)),
            )
          }
        }
      }
