...
import { SceneManager, type Scene } from '@/tower/core/SceneManager'
import { getApp } from '@/tower/core/AppContext'
import { getConfig, getAllItems, setRunClassItemPoolIds } from '@/tower/core/DataLoader'
import {
...
import { CELL_HEIGHT, CELL_SIZE } from '@/tower/common/grid/GridZone'
import type { ItemDef } from '@/tower/common/items/ItemDef'
import { parseTierName } from '@/tower/common/items/ItemTierStats'
import { ShopManager, getDailyGoldForDay, type TierKey } from '@/tower/shop/ShopManager'
import { getConfig as getDebugCfg } from '@/tower/config/debugConfig'
import { PhaseManager } from '@/tower/core/PhaseManager'
import { clearBattleSnapshot, getBattleSnapshot, setBattleSnapshot, type BattleSnapshotBundle } from '@/tower/battle/BattleSnapshotStore'
import { PvpContext } from '@/tower/pvp/PvpContext'
// getOpponentFromAlive moved to PvpPanel.ts
import { clearBattleOutcome, consumeBattleOutcome } from '@/tower/battle/BattleOutcomeStore'
...
import { getSkillItemDefById, getSkillPickById } from '@/tower/common/skills/SkillItemDefs'
import {
  stopFlashEffect,
...
import { getShopUiColor, getClassColor } from '@/tower/config/colorPalette'
...
function seedInitialUnlockPoolByStarterClass(pick: import('./ShopSceneContext').StarterClass, ctx: ShopSceneCtx = _ctx): void {
...
function ensureStarterClassSelection(stage: Container, ctx: ShopSceneCtx = _ctx): void {
  ...
  HeroSystem.ensureStarterClassSelection(ctx, stage, {
    ...
    refreshShopUI: () => refreshShopUI(ctx),
    captureAndSave: makeCaptureAndSave(ctx),
    ensureDailyChoiceSelection: (s) => ensureDailyChoiceSelection(s, ctx),
    grantHeroStartDayEffectsIfNeeded: () => HeroSystem.grantHeroStartDayEffectsIfNeeded(ctx, {
    ...
    grantStarterItemsByClass: (pick) => HeroSystem.grantStarterItemsByClass(ctx, pick, {
      findFirstBattlePlace: (size) => findFirstBattlePlace(size, ctx),
      findFirstBackpackPlace: (size) => findFirstBackpackPlace(size, ctx),
      nextId: () => nextId(),
      ...
      syncUnlockPoolToManager: () => syncUnlockPoolToManager(),
      instanceToDefIdSet: (id, defId) => instanceToDefId.set(id, defId),
      instanceToPermanentDamageBonusSet: (id, value) => instanceToPermanentDamageBonus.set(id, value),
    }),
  })
}
...
function refreshShopUI(ctx: ShopSceneCtx = _ctx): void {
  ...
  if (ctx.livesText) {
    if (towerMode) {
      ctx.livesText.visible = false
    } else if (PvpContext.isActive()) {
