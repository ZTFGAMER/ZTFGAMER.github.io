...
import { getAllItems, getAllItemsRaw, getConfig, isRunClassItemPoolAllowed, setRunClassItemPoolIds } from '@/tower/core/DataLoader'
import { getLifeState, getPlayerProgressState, markRunStarted, setLifeState } from '@/tower/core/RunState'
import { normalizeSize, type ItemDef } from '@/tower/common/items/ItemDef'
import { resolveItemTierBaseStats } from '@/tower/common/items/ItemTierStats'
...
import { getHeroImageUrl, getItemIconUrl } from '@/tower/core/AssetPath'
import { createItemStatBadges } from '@/tower/common/ui/ItemStatBadges'
import { getTierColor, getClassColor } from '@/tower/config/colorPalette'
import { calcSkill94DailyGoldBonus } from '@/tower/common/skills/GoldSkillRules'
import {
...
const TOWER_STARTER_CLASS_ORDER: StarterClass[] = ['swordsman', 'archer', 'assassin']
...
export function ensureStarterClassSelection(
  ...
  callbacks: {
    ...
    grantStarterItemsByClass: (pick: StarterClass) => void
  },
): void {
  ...
  const order: StarterClass[] = [...TOWER_STARTER_CLASS_ORDER]
  ...
  let selected: StarterClass | null = order.includes(ctx.starterClass as StarterClass)
    ? ctx.starterClass
    : null
  ...
  const confirmSelection = () => {
    ...
    callbacks.grantStarterItemsByClass(selected)
