...
import { getConfig as getDebugCfg, setConfig as setDebugCfg } from '@/tower/config/debugConfig'
import { getTopLeftControlYOffset } from '@/tower/shop/ui/ShopSafeArea'
import { getAllItemsRaw, getConfig } from '@/tower/core/DataLoader'
...
import { getItemIconUrl, getSkillIconUrl } from '@/tower/core/AssetPath'
import { BRONZE_SKILL_PICKS } from '@/tower/common/skills/BronzeSkillConfig'
import { SILVER_SKILL_PICKS } from '@/tower/common/skills/SilverSkillConfig'
import { GOLD_SKILL_PICKS } from '@/tower/common/skills/GoldSkillConfig'
import { getAllSkillItemDefs, getSkillIconStemById, isSkillItemDefId } from '@/tower/common/skills/SkillItemDefs'
...
export class SettingsDebugPanel extends Container {
  ...
  openItemCompendiumOverlay(): void {
    ...
    const runPoolItems = cb.getAllItems().filter((it) => !cb.isNeutralArchetypeKey(cb.getPrimaryArchetype(it.tags)))
    ...
    const countByTierAndArchetype = (tier: 'Bronze' | 'Silver' | 'Gold' | 'Diamond', archetype: '战士' | '弓手' | '刺客'): number => {
      return runPoolItems.filter((it) => {
        const itemTier = parseTierName(it.starting_tier) ?? 'Bronze'
        return itemTier === tier && cb.getPrimaryArchetype(it.tags) === archetype
      }).length
    }
    const tierDefs = [
      { key: 'Bronze', count: Math.max(...archetypeRows.map((row) => countByTierAndArchetype('Bronze', row.key))) },
      { key: 'Silver', count: Math.max(...archetypeRows.map((row) => countByTierAndArchetype('Silver', row.key))) },
      { key: 'Gold', count: Math.max(...archetypeRows.map((row) => countByTierAndArchetype('Gold', row.key))) },
      { key: 'Diamond', count: Math.max(...archetypeRows.map((row) => countByTierAndArchetype('Diamond', row.key))) },
    ] as const
