...
import { SHOP_STATE_STORAGE_KEY } from '@/tower/core/RunState'
import { getAllItems, getRunClassItemPoolIds, setRunClassItemPoolIds } from '@/tower/core/DataLoader'
import type { ShopSlot } from '@/tower/shop/ShopManager'
import type { GridSystem } from '@/tower/common/grid/GridSystem'
import type { GridZone } from '@/tower/common/grid/GridZone'
...
export type ApplySavedShopStateCallbacks = {
  toVisualTier: (tier?: import('@/tower/shop/ShopManager').TierKey, star?: 1 | 2) => string | undefined
  ...
  ctx.pendingHeroPeriodicRewards = Array.isArray(state.pendingHeroPeriodicRewards)
    ? state.pendingHeroPeriodicRewards
      .map((row) => {
        ...
        const tier = (String(row?.tier ?? 'Bronze') as import('@/tower/shop/ShopManager').TierKey)
        ...
        const picks = Array.isArray(entry?.picks)
          ? entry.picks
            .map((pick) => {
              ...
              const tier = (String(pick?.tier ?? 'Bronze') as import('@/tower/shop/ShopManager').TierKey)
