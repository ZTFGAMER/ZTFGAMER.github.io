export const SHOP_STATE_STORAGE_KEY = 'bigbazzar_tower_shop_state_v1'

const LIFE_STATE_STORAGE_KEY = 'bigbazzar_tower_life_state_v1'
const LIFE_STATE_STORAGE_VERSION = 1
const DEFAULT_MAX_LIVES = 30
const TROPHY_STATE_STORAGE_KEY = 'bigbazzar_tower_trophy_state_v1'
const TROPHY_STATE_STORAGE_VERSION = 1
const DEFAULT_WIN_TARGET = 10
const WIN_STREAK_STORAGE_KEY = 'bigbazzar_tower_win_streak_v1'
const WIN_STREAK_STORAGE_VERSION = 1
const PLAYER_PROGRESS_STORAGE_KEY = 'bigbazzar_tower_player_progress_v1'
const PLAYER_PROGRESS_STORAGE_VERSION = 1
const LAST_STAND_STATE_STORAGE_KEY = 'bigbazzar_tower_last_stand_state_v1'
const LAST_STAND_STATE_STORAGE_VERSION = 1
const PLAYER_RUN_META_STORAGE_KEY = 'bigbazzar_tower_player_run_meta_v1'
const PLAYER_RUN_META_STORAGE_VERSION = 1
const TOWER_ENDLESS_RECORD_STORAGE_KEY = 'bigbazzar_tower_endless_record_v1'
const TOWER_ENDLESS_RECORD_STORAGE_VERSION = 1
...
export type TowerEndlessRecordState = {
  bestDay: number
}
...
function clampTowerEndlessRecord(bestDay: unknown): TowerEndlessRecordState {
  const n = Number(bestDay)
  return {
    bestDay: Number.isFinite(n) ? Math.max(1, Math.round(n)) : 1,
  }
}

function saveTowerEndlessRecordState(state: TowerEndlessRecordState): TowerEndlessRecordState {
  const normalized = clampTowerEndlessRecord(state.bestDay)
  try {
    localStorage.setItem(TOWER_ENDLESS_RECORD_STORAGE_KEY, JSON.stringify({
      version: TOWER_ENDLESS_RECORD_STORAGE_VERSION,
      state: normalized,
    }))
  } catch {
    // ignore
...
export function getTowerEndlessRecordState(): TowerEndlessRecordState {
  try {
    const raw = localStorage.getItem(TOWER_ENDLESS_RECORD_STORAGE_KEY)
    if (!raw) return saveTowerEndlessRecordState({ bestDay: 1 })
    const parsed = JSON.parse(raw) as {
      version?: unknown
      state?: { bestDay?: unknown }
    } | null
    if (!parsed || parsed.version !== TOWER_ENDLESS_RECORD_STORAGE_VERSION || !parsed.state) {
      return saveTowerEndlessRecordState({ bestDay: 1 })
    }
    return saveTowerEndlessRecordState(clampTowerEndlessRecord(parsed.state.bestDay))
  } catch {
    return saveTowerEndlessRecordState({ bestDay: 1 })
  }
}

export function recordTowerEndlessReachedDay(day: number): TowerEndlessRecordState {
  const current = getTowerEndlessRecordState()
  const nextDay = Math.max(1, Math.round(Number(day) || 1))
  if (nextDay <= current.bestDay) return current
  return saveTowerEndlessRecordState({ bestDay: nextDay })
}
