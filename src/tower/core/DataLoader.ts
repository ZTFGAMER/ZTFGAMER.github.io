...
import type { GameConfig, ItemDef } from '@/tower/common/items/ItemDef'

// Vite 支持直接 import JSON（resolveJsonModule）
import rawConfig   from '../../../data/tower_game_config.json'
import rawItems    from '../../../data/tower_items.json'
import rawItemEnchantments from '../../../data/tower_item_enchantments.json'
...
function validateCombatRuntime(v: unknown): void {
  ...
  const required = [
    ...
    'burnShieldFactor', 'burnDecayPct', 'healCleansePct', 'ammoAutoReloadMs',
    ...
    'itemValueTweenMs', 'itemValuePulseUpMs', 'itemValuePulseDownMs', 'itemValuePulsePeakScale',
...
function extractConfig(entries: ConfigEntry[]): GameConfig {
  ...
  const cfg = {
    ...
    towerDefenseRules:  getOptional<GameConfig['towerDefenseRules']>('tower_defense_rules'),
  }
