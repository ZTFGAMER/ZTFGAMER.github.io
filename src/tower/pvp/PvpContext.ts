...
import { getBattleSnapshot, setBattleSnapshot } from '@/tower/battle/BattleSnapshotStore'
import { consumeBattleOutcome } from '@/tower/battle/BattleOutcomeStore'
import { setPvpPlayerProgressOverride } from '@/tower/core/RunState'

import type { PvpSession } from '@/tower/pvp/PvpTypes'
import type { PvpRoom } from '@/tower/pvp/PvpRoom'
...
let pendingOpponentSnap: import('@/tower/battle/BattleSnapshotStore').BattleSnapshotBundle | null = null
...
let lastPlayerSnapshots: Record<number, import('@/tower/battle/BattleSnapshotStore').BattleSnapshotBundle> = {}
...
export const PvpContext = {
  ...
  startSession(pvpRoom: PvpRoom, pvpSession: PvpSession): void {
    ...
    pvpRoom.onError = (msg) => {
      ...
      if (cur === 'tower-shop' || cur === 'tower-battle') {
    ...
    pvpRoom.onOpponentSnapshot = (day, opponentSnap, opponentPlayerIndex, authoritativeWinner) => {
      ...
      if (SceneManager.currentName() !== 'tower-shop') {
    ...
    pvpRoom.onGameOver = (rankings) => {
      ...
      // 胜者在 onBattleComplete 里已 advanceToDay 并 goto('tower-shop')，此时需主动跳转结算
  ...
  getLastPlayerSnapshots(): Record<number, import('@/tower/battle/BattleSnapshotStore').BattleSnapshotBundle> {
  ...
  /** BattleScene 退出过渡结束时调用（替代 SceneManager.goto('tower-shop')） */
  onBattleComplete(): void {
    ...
    // client 路径：onRoundSummary 可能已更新 eliminatedPlayers，若已淘汰则不再 goto('tower-shop')
